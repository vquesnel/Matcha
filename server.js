...
connection.query("CREATE TABLE IF NOT EXISTS `matcha`.`notification` (`id` INT(5) NOT NULL AUTO_INCREMENT, `sender` VARCHAR(255) NOT NULL , `sended` VARCHAR(255) NOT NULL, `content` VARCHAR(255) NOT NULL, `date` DATE NOT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_bin;");
...
app.get('/search', function (req, res) {
	if (!req.session.username) {
		...
		if (req.query.submit) {
			...
			var data = [];
			connection.query('SELECT firstname,lastname from users where (firstname like "%' + req.query.key + '%" OR lastname like "%' + req.query.key + '%") AND  username NOT IN (SELECT blocked FROM block WHERE block_by = ?)', [req.session.username], function (err, rows, fields) {
				if (err) throw err;
			...
			connection.query("SELECT tag FROM tags WHERE username= ?", [req.session.username], function (err, tags) {
				...
				if (firstword && lastword) {
					var tag_tofind = [];
					connection.query("SELECT username FROM users WHERE ((firstname = ? AND lastname = ?) OR (firstname = ? AND lastname = ?)) AND  username NOT IN (SELECT blocked FROM block WHERE block_by = ?)", [firstword, lastword, lastword, firstword, req.session.username], function (err, rows) {
						if (err) throw err;
						if (rows[0] && !rows[1]) {
							...
							connection.query('SELECT * from users where ((firstname like "%' + firstword + '%" AND lastname like "%' + lastword + '%") OR (firstname like "%' + lastword + '%" AND lastname like "%' + firstword + '%")) AND  username NOT IN (SELECT blocked FROM block WHERE block_by = ?)', [req.session.username], function (err, rows) {
								if (err) throw err;
								if (!rows[0]) {
									res.render("search.html", {
					...
					connection.query("SELECT username FROM users WHERE (firstname = ?  OR  lastname = ?) AND  username NOT IN (SELECT blocked FROM block WHERE block_by = ?)", [firstword, firstword, req.session.username], function (err, rows) {
						if (err) throw err;
						if (rows[0] && !rows[1]) {
							if (rows[0].username === req.session.username) {
								res.redirect('/profile.html');
							...
							connection.query('SELECT * from users where (firstname like "%' + firstword + '%"  OR  lastname like "%' + firstword + '%") AND  username NOT IN (SELECT blocked FROM block WHERE block_by = ?)', [req.session.username], function (err, rows) {
								if (err) throw err;
								if (!rows[0]) {
									res.render("search.html", {
										message: "Nobobdy match this name"
...
app.get('/users.html/:user', function (req, res) {
	...
	if (!req.session.username) {
		...
		if (req.params.user === req.session.username) {
			...
			connection.query("INSERT INTO notification(sender,sended,content, date) VALUES(?,?,?, ?)", [req.session.username, req.params.user, content, new Date()], function (err) {
				if (err) throw err;
			})
...
app.get("/notification.html", function (req, res) {
	if (!req.session.username) {
		...
		connection.query("SELECT * FROM notification WHERE sended = ? ORDER by date", [req.session.username], function (err, rows) {
			if (err) throw err;
...
io.on('connection', function (socket) {
	...
	socket.on("delete Notification", function () {
		connection.query("DELETE FROM notification WHERE sended = (SELECT username FROM users WHERE sessionID= ?)", [sessionid], function (err) {
			if (err) throw err;
		})
	})
});
